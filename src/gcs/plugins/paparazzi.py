...
class PaparazziControlPanel(AbstractControlPanel):

    __mavlinkMessageTypes = []
    cmdSent = 0
    ...
    def __init__(self, parent = None):
        ...
        l = QGridLayout()
        row = 0
        l.addWidget(QLabel('Paparazzi Tools'), row, 0, 1, 3, Qt.AlignLeft)
        row += 1
        self.accelCalibButton = QPushButton('Accel Calib.')
        self.paramDefaultButton = QPushButton('Reset Parameters')
        self.paramSaveButton = QPushButton('Save Parameters')
        self.paramReadButton = QPushButton('Read Parameters')
        ...
        self.paramSaveButton.clicked.connect(lambda: \
            self.__sendMAVLinkLongMessage(command=mavlink.MAV_CMD_PREFLIGHT_STORAGE,
                                          param1=1))

        self.paramReadButton.clicked.connect(lambda: \
            self.__sendMAVLinkLongMessage(command=mavlink.MAV_CMD_PREFLIGHT_STORAGE,
                                          param1=0,
                                          syncParams=True))

        l.addWidget(self.paramSaveButton, row, 0, 1, 1, Qt.AlignLeft)
        l.addWidget(self.paramReadButton, row, 2, 1, 1, Qt.AlignLeft)
        row += 1
        l.addWidget(self.accelCalibButton, row, 0, 1, 1, Qt.AlignLeft)
        l.addWidget(self.paramDefaultButton, row, 2, 1, 1, Qt.AlignLeft)
        row += 1
